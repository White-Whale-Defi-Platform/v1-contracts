...
impl Deposit {
    ...
    pub fn decrease(&mut self, amount: Uint128) -> StdResult<Self> {
        if amount > self.value {
            return Err(StdError::generic_err(format!("Cannot decrease {} by {}", self.value, amount)));
        }
...
impl UserDeposit<'_> {
    pub const fn new() -> UserDeposit<'static> {
        UserDeposit{ map: Map::new("deposits")}
    }

    pub fn increase(&self, storage: &mut dyn Storage, key: &[u8], amount: Uint128) -> StdResult<()> {
        let user_deposit = &mut self.map.may_load(storage, key)?.unwrap_or_default();
        self.map.save(storage, key, &user_deposit.increase(amount))?;
        Ok(())
    }

    pub fn decrease(&self, storage: &mut dyn Storage, key: &[u8], amount: Uint128) -> StdResult<()> {
        let mut user_deposit: Deposit = self.map.may_load(storage, key)?.unwrap_or_default();
        self.map.save(storage, key, &user_deposit.decrease(amount)?)?;
...
impl DepositManager {
    ...
    pub fn increase(&self, storage: &mut dyn Storage, key: &[u8], amount: Uint128) -> StdResult<()> {
        ...
        self.total_deposits.save(storage, &total_deposits.increase(amount))?;
    ...
    pub fn decrease(&self, storage: &mut dyn Storage, key: &[u8], amount: Uint128) -> StdResult<()> {
        ...
        let mut total_deposits = self.total_deposits.load(storage)?;
        self.total_deposits.save(storage, &total_deposits.decrease(amount)?)?;
        Ok(())
    }
...
mod tests {
    ...
    fn test_user_deposits() {
        ...
        user_deposits.increase(&mut storage, key, Uint128::from(10u64)).unwrap();
        ...
        user_deposits.decrease(&mut storage, key, Uint128::from(5u64)).unwrap();
        let value = user_deposits.get(&mut storage, key).unwrap();
        assert_eq!(value, Uint128::from(15u64));

        user_deposits.decrease(&mut storage, key, Uint128::from(15u64)).unwrap();
        let value = user_deposits.get(&mut storage, key).unwrap();
    ...
    fn test_deposit_manager() {
        ...
        deposits.increase(&mut storage, key, Uint128::from(10u64)).unwrap();
        ...
        let value = deposits.get(&mut storage, key).unwrap();
        assert_eq!(value, Uint128::from(20u64));
        assert_eq!(deposits.get_total_deposits(&storage).unwrap(), Uint128::from(20u64));

        deposits.increase(&mut storage, other_key, Uint128::from(10u64)).unwrap();
        let value = deposits.get(&mut storage, key).unwrap();
        assert_eq!(value, Uint128::from(20u64));
        let value = deposits.get(&mut storage, other_key).unwrap();
        assert_eq!(value, Uint128::from(10u64));
        assert_eq!(deposits.get_total_deposits(&storage).unwrap(), Uint128::from(30u64));

        let res = deposits.decrease(&mut storage, other_key, Uint128::from(15u64));
        assert!(res.is_err());

        deposits.decrease(&mut storage, key, Uint128::from(15u64)).unwrap();
        assert_eq!(deposits.get_total_deposits(&storage).unwrap(), Uint128::from(15u64));

        deposits.decrease(&mut storage, key, Uint128::from(5u64)).unwrap();
        assert_eq!(deposits.get_total_deposits(&storage).unwrap(), Uint128::from(10u64));

        deposits.decrease(&mut storage, other_key, Uint128::from(10u64)).unwrap();
        assert_eq!(deposits.get_total_deposits(&storage).unwrap(), Uint128::from(0u64));
    }
}
