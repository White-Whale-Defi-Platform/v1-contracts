...
use cosmwasm_std::{Env, Deps, Uint128,Decimal, StdResult, Addr,Fraction, CanonicalAddr};
...
pub fn proxy_value(
    ...
    match proxy_pool {
        ...
        Some(pool_addr) => {
            ...
            // holding = #bluna
            let pool_info: PoolResponse = query_pool(deps, pool_addr)?;
            let proxy_holding: Uint128;
            let proxy_asset_info: AssetInfo;
            ...
            return proxy_asset.value(deps, env, Some(proxy_holding));
        }
        ...
        None => {
            ...
            return proxy_vault_asset.value(deps, env, None);
        }
    }
}
