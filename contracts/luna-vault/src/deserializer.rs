...
pub fn deserialize_key<K: KeyDeserialize>(key: Vec<u8>) -> VaultResult<K::Output> {
    K::from_vec(key)
}
...
pub trait KeyDeserialize {
    ...
    fn from_vec(value: Vec<u8>) -> VaultResult<Self::Output>;
}
...
impl KeyDeserialize for u64 {
    ...
    fn from_vec(value: Vec<u8>) -> VaultResult<Self::Output> {
        Ok(u64::from_be_bytes(value.as_slice().try_into().map_err(
            |err: TryFromSliceError| LunaVaultError::generic_err(err.to_string()),
        )?))
    }
}
...
impl KeyDeserialize for String {
    ...
    #[inline(always)]
    fn from_vec(value: Vec<u8>) -> VaultResult<Self::Output> {
        Ok(String::from_utf8(value).map_err(StdError::invalid_utf8)?)
    }
}
