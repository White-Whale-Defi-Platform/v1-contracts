use cosmwasm_std::{ entry_point,
    to_binary, Binary, CosmosMsg, Decimal, Deps, DepsMut, Env, MessageInfo, Response,
    StdError, StdResult, Uint128, WasmMsg
};
use terraswap::asset::{Asset, AssetInfo};
use terraswap::pair::{ExecuteMsg as TerraswapMsg, Cw20HookMsg, PoolResponse};
...
use crate::state::{State, STATE, ADMIN, DEPOSIT_INFO, TRADER};
use crate::msg::{InitMsg, ExecuteMsg, QueryMsg, WithdrawableProfitsResponse};
...
pub fn instantiate(
    ...
    if msg.max_deposit.info != msg.min_profit.info {
        return Err(StdError::generic_err("Different asset infos for max_deposit and min_profit."));
    }
    let lp_token_addr = query_lp_token(deps.as_ref(), deps.api.addr_validate(&msg.terraswap_pool_addr)?)?;
    let state = State {
        ...
        if let AssetInfo::Token { contract_addr, .. } = &pool.info {
            ...
            messages.push(CosmosMsg::Wasm(WasmMsg::Execute{
                ...
                msg: to_binary(&Cw20ExecuteMsg::IncreaseAllowance{
                    spender: deps.api.addr_humanize(&state.terraswap_pool_addr)?.to_string(),
                    amount: deposits[i],
                    expires: None
                })?,
                funds: vec![]
...
fn withdraw(
    ...
    let cw20_msg = Cw20ExecuteMsg::Send{
        contract: deps.api.addr_humanize(&state.terraswap_pool_addr)?.to_string(),
        amount: lp_amount,
...
fn set_min_profit(
    ...
    Ok(Response::default().add_attribute("action", "set_min_profit").add_attribute("asset", asset.to_string()))
}
...
fn query_withdrawable_profits(deps: Deps, env: Env) -> StdResult<WithdrawableProfitsResponse> {
    let state = STATE.load(deps.storage)?;
    let pool: PoolResponse = query_pool(deps, deps.api.addr_humanize(&state.terraswap_pool_addr)?)?;
    let lp_balance: Uint128 = query_token_balance(&deps.querier, deps.api.addr_humanize(&state.lp_token_addr)?, env.contract.address)?;
    ...
    if total_value.amount > Uint128::zero() && total_value.amount > state.max_deposit.amount + state.min_profit.amount {
