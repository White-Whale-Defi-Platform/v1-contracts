...
use cosmwasm_std::{
    CanonicalAddr, HumanAddr, Binary, Coin, StdError, StdResult, SystemError, SystemResult, 
    AllBalanceResponse, AllDelegationsResponse, BalanceResponse, BankQuery, BondedDenomResponse,
    FullDelegation, QueryRequest, StakingQuery, Validator, ValidatorsResponse, 
    WasmQuery, from_slice, to_binary, MemoryStorage, Api, Extern, Querier, QuerierResult, 
    BlockInfo, ContractInfo, Empty, Env, MessageInfo, Uint128
};
...
impl DummyQuerier {
    fn process_smart_query(&self, contract_addr: &HumanAddr) -> QuerierResult {
        if contract_addr == &self.pool_address {
            let binary_response = to_binary(&SimulationResponse{ 
                return_amount: Uint128(1000000),
            ...
            return Ok(Ok(binary_response.unwrap()));
        }
    ...
    pub fn query(&self, request: &StakingQuery) -> QuerierResult {
        match request {
            ...
            StakingQuery::Delegation { .. } => {
                Err(SystemError::UnsupportedRequest{ kind: "not implemented".to_string() })
            }
        }
    }
}
...
mod test {
    use super::*;
    use cosmwasm_std::{coin, coins, from_binary, Decimal, Delegation, HumanAddr};
